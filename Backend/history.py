...
async def get_answered_questions(
    ...
    try:
        ...
        # Format response using the results from the RPC
        answered_questions = [
            AnsweredQuestionStats(
...
async def get_passed_lessons(
    ...
    try:
        ...
        lessons = [
            PassedLesson(lesson_id=lid)
            for lid in unique_lessons
        ]
    ...
    try:
        ...
        if not passed_data:
            ...
            if session_response.data:
                return NextSessionResponse(session_id=session_response.data[0]['id'])
            return NextSessionResponse(session_id=None)
        ...
        if next_session_response.data:
            return NextSessionResponse(session_id=next_session_response.data[0]['id'])
    ...
    try:
        ...
        if not passed_data:
            ...
            if lesson_response.data:
                return NextLessonResponse(lesson_id=lesson_response.data[0]['id'])
            return NextLessonResponse(lesson_id=None)
        ...
        if next_lesson_response.data:
            return NextLessonResponse(lesson_id=next_lesson_response.data[0]['id'])
